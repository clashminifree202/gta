...
async def gate_page():
    ...
    gate_path = os.path.join("dist", "gate.html")
    if not os.path.exists(gate_path):
        return Response("gate.html not found", status_code=404)
    with open(gate_path, "r", encoding="utf-8") as f:
        return Response(f.read(), media_type="text/html")


@app.get("/gate/auth")
async def gate_auth(request: Request, key: str = ""):
    if not GATE_ENABLED:
        return RedirectResponse("/")
    if key.strip() and key == GATE_PASSWORD:
        ...
        return response
    return Response("Bad gate key", status_code=401)


@app.get("/")
async def read_index(request: Request):
    if GATE_ENABLED and request.cookies.get(GATE_COOKIE) != GATE_HASH:
